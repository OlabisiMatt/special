...
if(isset($_POST['submit'])){
    ...
    $select_email = $conn->prepare("SELECT * FROM `owner` WHERE  email = ?");
    ...
    if ($select_email->rowCount() > 0) {
        ...
        if($password != $c_password){
            ...
            $insert_user = $conn->prepare("INSERT INTO `owner`(id, firstname,
             lastname, email, phone, password) VALUES(?,?,?,?,?,?)");
            $insert_user->execute([$id, $firstname, $lastname, $email, $phone, $c_password]);
...
<body>
    ...
    <section class="form-container">
        <form action="" method="POST">
            ...
            class="box">
            <p>already have an account? <a href="ologin.php">login now</a></p>
